...
public class Pastebin {
     ...
     */
    public static URL pastePaste(String developerKey,
                                 String contents) throws PasteException {
        return pastePaste(developerKey,
                          contents,
                          null);
    }
     ...
     * @return a new paste
     */
    public static PastebinPaste newPaste(String developerKey,
                                         String contents,
                                         String title) {
        PastebinPaste paste = new PastebinPaste(developerKey,
                                                contents);
    ...
    public static PastebinLink[] getTrending(String developerKey) throws ParseException {
        ...
        Post post = new Post();
        post.put("api_dev_key",
                 developerKey);
        post.put("api_option",
                 "trends");

        String response = Web.getContents(API_POST_LINK,
                                          post);
                ...
                for (int i = 0; i < nodes.getLength(); i++) {
                    ...
                    if (node.getNodeType() == Node.ELEMENT_NODE) {
                        ...
                        String pasteFormat = XMLUtils.getText(element,
                                                              "paste_format_short");
                        String title = XMLUtils.getText(element,
                                                        "paste_title");
                        int visibility = Integer.parseInt(XMLUtils.getText(element,
                                                                           "paste_private"));
                        int hits = Integer.parseInt(XMLUtils.getText(element,
                                                                     "paste_hits"));

                        long expireDate = Long.parseLong(XMLUtils.getText(element,
                                                                          "paste_expire_date"));
                        long pasteDate = Long.parseLong(XMLUtils.getText(element,
                                                                         "paste_date"));
     ...
     * </ul>
     * @param post the <code>Post</code> with the options
     * @return the pastes.
     ...
     * Public.
     * @param tempMap the Json Map
     * @return PastebinLink with all the informations
     * @throws MalformedURLException if there are problems with the paste's url
     * @author Felipe
     */
    private static PastebinLink jSonMapToPastebinLink(Map<String, Object> tempMap) throws MalformedURLException {
        ...
        PastebinLink pastebinLink = null;
        try {
            pastebinLink = new PastebinLink(pastebinPaste,
                                            new URL(tempMap.get("full_url").toString()),
                                            new Date(pasteDate * 1000));
     ...
     * @param response the pastebin JSON response
